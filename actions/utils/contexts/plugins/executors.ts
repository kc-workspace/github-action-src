...
import { toBool } from "@utils/converters"
...
export class ExecContextPlugin implements ContextPlugin<"exec"> {
  ...
  /**
   * call this function before {@link run()} to inject execute options
   */
  ...
  /**
   * run input commandline with optional options (from {@link withOptions()}).
   * This will reset options after completed. To preserve the options later,
   * use {@link rerun()} instead
   *
